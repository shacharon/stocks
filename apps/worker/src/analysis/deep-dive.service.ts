...
export class DeepDiveService {
    ...
    async generateReport(
        ...
        return {
            ...
            keyMetrics: {
                ...
                sma20: features.sma20 ? num(features.sma20) : null,
                sma50: features.sma50 ? num(features.sma50) : null,
                sma200: features.sma200 ? num(features.sma200) : null,
                rsi: features.rsi14 ? num(features.rsi14) : null,
                atr: features.atr14 ? num(features.atr14) : null,
                volumeRatio: features.volumeRatio ? num(features.volumeRatio) : null,
            },
            riskAssessment,
            recommendations,
            supportingData: {
    ...
    private analyzeTrend(current: any, historical: any[]): string {
        ...
        const price = num(current.closePrice);
        const sma20 = num(current.sma20);
        const sma50 = num(current.sma50);
        const sma200 = current.sma200 ? num(current.sma200) : null;
    ...
    private analyzeVolatility(current: any, historical: any[]): string {
        if (!current.atr14 || !current.closePrice) {
            return 'INSUFFICIENT_DATA';
        }

        const atr = num(current.atr14);
        const price = num(current.closePrice);
    ...
    private assessRisk(features: any, signal: string, confidence: number): {
        ...
        // Volatility risk
        if (features.atr14 && features.closePrice) {
            const atrPercent = (num(features.atr14) / num(features.closePrice)) * 100;
            if (atrPercent > 3) {
    ...
    async saveReport(report: DeepDiveReport): Promise<void> {
        await this.prisma.deepDiveReport.upsert({
            where: {
                symbol_market_date: {
    ...
    async getReport(symbol: string, market: Market, date: Date): Promise<any> {
        return this.prisma.deepDiveReport.findUnique({
            where: {
                symbol_market_date: { symbol, market, date },
            },
        });
    ...
    async getReportsForDate(date: Date, market?: Market): Promise<any[]> {
        return this.prisma.deepDiveReport.findMany({
            where: {
                date,
    ...
    async getReportStats(): Promise<{
        ...
        const [totalReports, earliest, latest, bySignal] = await Promise.all([
            ...
            this.prisma.deepDiveReport.findFirst({
                orderBy: { date: 'asc' },
                select: { date: true },
            }),
            this.prisma.deepDiveReport.findFirst({
            ...
            this.prisma.deepDiveReport.groupBy({
                by: ['signal'],
                _count: true,
            }),
        ]);
